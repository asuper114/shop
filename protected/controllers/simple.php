    ...
    public function reg_act(){
        if($this->getModule()->checkToken('reg')){
            ...
            if($smsCheck){
                ...
                if($obj) $checkFlag = ( $mobile_code == $obj['code']);
        ...
        if($obj){
            if($obj['status']==1){
                if($obj['password'] == CHash::md5($passWord,$obj['validcode'])){
                    ...
                    if($redirectURL!='' && preg_match("/https?:\/\//i",$redirectURL)==0 && stripos($redirectURL, "reg")===false && stripos($redirectURL, "login_act")===false&& stripos($redirectURL, "activation_user")===false && stripos($redirectURL, "reset_password_act")===false)header('Location: '.$redirectURL, true, 302);
